...
class CurrentProjectSyncTests(unittest.TestCase):
    ...
    def test_old_stamp_routes_to_confirmed_rebuild_and_preserves_manifest(self) -> None:
        ...
        navigation_managed_row = (
            "| 当前交付 | [`1_delivery/`](1_delivery/)；"
            "从 `lifecycle: active` 的交付包继续 |\n"
        )
        navigation_project_row = (
            "| 当前项目主线 | [`project_topic/`](1_delivery/project_topic/) |\n"
        )
        self.assertIn(navigation_managed_row, doc_readme)
        legacy_navigation_row = navigation_managed_row.replace(
            "从 `lifecycle: active` 的交付包继续",
            "OLD-MANAGED-NAVIGATION",
        )
        doc_readme = doc_readme.replace(
            navigation_managed_row,
            legacy_navigation_row + navigation_project_row,
            1,
        )
        managed_row = (
        ...
        self.assertIn("## 从这里开始", rebuilt_readme)
        self.assertIn("## 文档生命周期", rebuilt_readme)
        self.assertIn("缺少 `lifecycle` 的事项视为 `unclassified`", rebuilt_readme)
        self.assertIn(navigation_managed_row.strip(), rebuilt_readme)
        self.assertIn(navigation_project_row.strip(), rebuilt_readme)
        self.assertNotIn("OLD-MANAGED-NAVIGATION", rebuilt_readme)
