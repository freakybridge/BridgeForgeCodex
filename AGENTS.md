...
- BridgeForge 自带工具与 Hook 使用受管 Rust workspace；项目自有 Hook 可使用其目录内的独立 Cargo 工程。两者均使用锁定的 Cargo.lock 和受管构建入口；工具链缺失、版本不足、锁文件漂移或构建失败时停止，不回退到 Python、脚本包装器或其他未受管入口。
