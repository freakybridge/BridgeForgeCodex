...
- 执行类任务默认接管到结果：读上下文 -> 判断风险 -> 修改 / 执行 -> 验证 -> 汇报；交付必须说明“已做什么 / 验证了什么 / 还剩什么风险”，禁止用“你可以……”代替结果。§3.3 触发范围澄清时，该流程只能从用户回答后开始，禁止用“读上下文”跳过硬闸。
...
- 主对话负责与用户沟通、取得确认、处理授权和汇总结果；子 agent 只执行分配阶段，不代替用户决策，也不重复已完成工作。
- Skill 定义工作流程。
- 用户未明确要求、且适用的项目或 Skill 指令未显式委派的阶段，必须由主对话执行；Skill 一旦委派，必须点名已存在的 Agent 角色，禁止只写“独立 agent”“子 agent”等泛称。
- `$bridgeforge-codex`、`$create-worktree` 和 `$git-sync` 始终由主对话执行；`$git-sync` 只能运行当前项目自带的同步脚本。

### 3.3 较大需求主动澄清

琐碎、续接或细节完整的任务必须直接执行；新的、覆盖整个项目或多个子系统且没有说明具体结果、优先级或验收的请求，必须先只给当前理解并问一个最关键的范围问题，本轮停止。此范围硬闸先于读取仓库、调用工具、运行测试、写盘和进入 `$confirm` / `$plan` / `$develop`；只有用户回答后才能继续。其他关键取舍不明确的开发需求，仅在缺失信息会实质改变实现或验收时进入澄清。完整响应流程见 `doc/3_reference/codex-hook-signals.md`。
...
- 同一 Bug 前 3 次修改失败后，第 4 次禁止继续写；再次修改前必须取得量化证据，并列出已试方案和未验证假说，进入 `$escalate` 或 `$debate`。
