...
- 两条升级路径都在事务内精确退役旧 `.codex/find-doc.map.md` 与 `.codex/sync-docs.map.md`；新 Hook 在 `.runtime/bridgeforge-codex/` 按当前项目事实重建运行时索引，禁止把这些骨架内生文件加入 Git 或作为 project-owned 数据保留。
