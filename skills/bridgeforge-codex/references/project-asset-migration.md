...
7. 只有当前源确认后才能进入下一个；用户停止时停止迁移。恢复后先核对可见对话中的完整迁移包、明确授权、源 hash、目标原值和最新基线。仍可完整核实且未变化的决定继续有效，从未完成处恢复；授权已撤回、决定无法完整恢复或现场变化时，只重新确认受影响的源及共享目标相关包，不从摘要推定删除授权。全部源确认前仍禁止写项目文件、`.runtime`、临时 manifest 或其他恢复点；不新增持久恢复状态，Apply 前仍须以同一最终 manifest 重新规划并核验 fingerprint。
