...
**层标签**（每条 entry 前缀，让下游一眼看懂该不该 sync，语义见 `AGENTS.md` 项目级专区）：
...
- [product][repo][meta] 修复 Skill 账本 BOM 读取误报

## [1.17.7] - 2026-09-08

### Fixed

- [product] Skill 账本检查兼容 UTF-8 BOM，避免误报合法账本无法读取；保留损坏 JSON、格式错误和 Skill 内容漂移检查，不改写用户账本。

## [1.17.6] - 2026-09-08

### Changed

- [meta] 记录正式安装与原生记忆自动同步验收

## [1.17.5] - 2026-09-08

### Fixed

- [product][repo][meta] 修复原生记忆自动同步并支持项目 Hook 独立依赖

## [1.17.4] - 2026-09-08

### Fixed

- [product] Native Memory 自动恢复损坏的后台任务与 pending 登记并保留原件；Windows 使用进程出生时间识别 PID 复用，避免长期误判任务仍在运行。
- [product] SessionStart 接入去重失败提醒；状态损坏或基线缺失不再显示健康，busy 不再抹掉失败收据；基线更新中断可按可信收据恢复。

## [1.17.3] - 2026-09-08

### Fixed

- [product] Native Memory 在 Windows 原子替换文件时使用规范化长路径，避免深层同步快照误报路径不存在。
- [repo] Memory 后台句柄测试隔离其他并行测试的句柄继承，并等待探针进程完全退出后清理。

## [1.17.2] - 2026-09-08

### Fixed

- [product] 独立项目 Hook 支持 build.rs 在本次临时 OUT_DIR 生成的源码，按编译依赖清单捕获实际引用文件并核验自检期间内容稳定；继续拒绝工程外文件、输出目录外文件及链接绕过，旧单文件模式不放宽。

## [1.17.1] - 2026-09-08

### Fixed

- [product] 项目 Hook 保留路径统一按大小写无关方式校验，阻止 `.CARGO` 等写法绕过工作区、待写入文件与暂存区检查；相似但不同的名称仍可使用。

## [1.17.0] - 2026-09-08

### Added

- [product] 项目 Rust Hook 支持项目自有 Cargo.toml、Cargo.lock 和完整源码目录，额外依赖无需修改骨架；保留旧单文件模式，独立工程输入纳入构建指纹、暂存区检查和漂移阻断，继续锁定构建、Windows 无窗口入口及事务安装。

## [1.16.0] - 2026-09-07

### Added

- [product][repo][meta] 增加持续高耗能提醒并统一待办记录入口

## [1.15.0] - 2026-09-07

### Added

- [product] Stop Hook 按主对话持久记录连续高耗能回合：GPT-6 Astra 的 High 及以上强度，或 Fast，每满 10 轮显示一次非阻断提醒；重复回调不重计，普通档和未知状态中断连续性。只读获取实际回合设置，不自动调整模型、强度或速度，不调用额外模型。Fast 日志适配失败时记录未知。

## [1.14.22] - 2026-09-07

### Changed

- [product] todo 固定写入项目 `doc/0_architecture/TODO-INDEX.md`，沿用已有格式和编号；缺失时创建并登记索引，重复事项精确更新，未确认想法仅记录待确认，不再分流创建 Bug 或 delivery 文档。
...
- [product][repo][meta] 固化原生 Memory 本机索引与同步边界

## [1.14.19] - 2026-09-07

### Fixed

- [product][repo][meta] 将 Codex 自维护的根 `MEMORY.md`、`memory_summary.md`、`raw_memories.md` 固化为本机文件：新快照、远端发布、三方基线和冲突选择均排除它们，恢复时事务保留本机字节；旧快照继续完整验真后兼容投影，其余 Memory 文件保持不透明字节合并同步。

## [1.14.18] - 2026-09-06

### Fixed

- [product][repo][meta] 精简骨架协作流程并修复快照与Hook检查

## [1.14.17] - 2026-09-06

### Fixed

- [product] develop 的 L 级顺序任务默认由主对话复用调研并实施，取消固定的调研、实现 Agent 交接，保留独立审计；普通任务的交付收据区分无需审计与必需审计尚未完成。
- [product] sync-docs 默认由主对话定位文档，仅在独立任务及授权、预算允许时委派，保留只读预览和原文核对边界。
- [meta] 课题五复用现有行为用例与交付记录；规则一致性与模型行为分别验收，预算及 summary 简化仍待证据。

## [1.14.16] - 2026-09-06

### Fixed

- [product] Hook 调度器保留 Map 标脏与重建失败的诊断、失败码和直接检索回退提示，其余编辑检查与快照仍继续；同一编辑事件的编码和指令公共检查只执行一次，保留全部目标的专项检查。
- [product] apply_patch 虚拟事件保留逐文件、逐修改块的新侧内容与新增行范围，使坏味道软提醒覆盖实际新增代码，不从删除内容、未改动上下文或跨块拼接制造提示。
- [product] 跨项目写入拦截说明真实的目标项目执行路径，不再要求用户确认后原样重复重试；现有安全判定保持。
- [meta] 课题四复用定向测试与交付记录，区分扫描次数、小夹具耗时及尚未完成的真实自动触发验证。

## [1.14.15] - 2026-09-06

### Fixed

- [product] 快照与启动状态使用同一次 Git 查询，保留 HEAD、未知状态和无上游的区别；快照文件以独占创建避免同秒覆盖，自动清理仅作用于可识别的自动状态记录，保留手动交接和未知旧文件。
- [product] 启动提示与 resume 共用只读交接选择入口；snapshot 使用生成收据的确切路径，resume 核验 HEAD 及下一步相关文件，旧快照缺字段不再被当作状态一致。
- [meta] 课题三复用现有测试与行为场景记录，区分定向验证、模型对照和用户级安装及下游传播。

## [1.14.14] - 2026-09-06

### Fixed

- [product] 将 find-doc 输出规则合并回单文件入口，退役旧格式 reference 及其分发登记，移除旧交付路径、失效 rules 字典与固定检索工具名；状态结论读取实际文档字段和正文，不从目录推断完成。
- [meta] 在既有交付主题记录课题二的检索链修复、定向核验与尚未安装和行为对照的边界。

## [1.14.13] - 2026-09-05

### Changed

- [product] 精简公共 AGENTS，按授权、证据、环境、信息承载和验证组织常驻规范；复用有效决定与证据，取消固定检索工具假设，完整迁移及总结流程由已有 Skill 承接。
- [product] 迁移中断后核验可见的完整决定和源/目标现场，仅重核失效部分；保留全部确认前零写入、完整 manifest、指纹检查和事务回滚，不新增恢复状态。
- [product] 指令检查使用受管公共区完整 hash，不再重复维护公共章节标题清单；保留区域标记、项目标题、编码、围栏和镜像检查。
- [repo][meta] 同步工厂公共区、指令架构说明和上游更新手册，补充个人/骨架组合行为用例；用例静态检查不代表模型行为验证通过。个人 AGENTS 由用户单独授权修改，不纳入骨架分发。

## [1.14.12] - 2026-09-05

### Changed

- [product][repo][meta] 校准骨架执行边界并补齐升级验证

## [1.14.11] - 2026-09-05

### Fixed

- [product] 调研与审计角色严格只读、职责不暗示算力；安全退役机械 Git 子角色，未知内容或项目引用阻断升级，漂移与失败保留原文件。
- [repo] 角色与 Skill 维护约束对齐当前分发登记，移除过期中央 routing 要求。
- [repo][meta] 复用真实合同测试补组合升级与幂等断言，新增隔离行为用例；用例校验不代表模型对照已执行。

## [1.14.10] - 2026-09-05

### Fixed

- [product] 骨架入口先分流咨询、诊断与维护；只读查询和文档预览不触发产品更新或索引刷新。

## [1.14.9] - 2026-09-05

### Fixed

- [product] 精简并统一入口与 Skill 的取证、授权继承、澄清及暂停恢复规则，取消重复开工确认。

## [1.14.8] - 2026-09-05

### Changed

- [meta] 记录原生记忆自动同步全生命周期验收证据

## [1.14.7] - 2026-09-05

### Fixed

- [product][repo][meta] 修复原生记忆后台同步的句柄继承与关闭超时

## [1.14.6] - 2026-09-05

### Fixed

- [product][repo][meta] 修复原生记忆 Hook 的 PowerShell 启动语义

## [1.14.5] - 2026-09-05

### Fixed

- [product][repo][meta] 迁移原生记忆旧版官方 Hook

## [1.14.4] - 2026-09-05

### Fixed

- [product][repo][meta] 修复原生记忆快照排序兼容与自动同步

## [1.14.3] - 2026-09-05

### Fixed

- [product][repo][meta] 修复受管 Markdown 多表误判

## [1.14.2] - 2026-09-05

### Fixed

- [product][repo][meta] 修复干净工厂同步跳过运行时自愈

## [1.14.1] - 2026-09-04

### Fixed

- [product][repo][meta] 项目 Map 移入 runtime 并退役 Git 跟踪
...
- [product][repo][meta] 修复原生 Memory Hook 的 Windows 路径分隔符误报

## [1.12.4] - 2026-09-03

### Fixed

- [product][repo][meta] 修复目录级 AGENTS 文档检查并记录构建诊断

## [1.12.3] - 2026-09-03

### Fixed

- [product] 修复文档结构检查误拒绝 `doc/AGENTS.md`、导致 Git 提交被拦截的问题；允许普通文件形式的目录级指令，同名目录、其他未允许的文件和第七层目录仍会被拒绝。
- [repo] 同步工厂镜像、产品版本及分发清单，补充 flat / milestone 布局与非法顶层条目的回归测试。

## [1.12.2] - 2026-09-03

### Fixed

- [product][repo][meta] 修复项目 Hook 登记与 Codex 配置兼容性

## [1.12.1] - 2026-09-03

### Fixed

- [product] 项目 Rust Hook 登记迁入项目所有的 `.codex/project-hooks.json`；原生 `hooks.json` 不再携带 Codex 不接受的骨架顶层字段。升级事务支持旧登记迁移、双份冲突阻断和输入漂移检查，暂存区校验读取独立登记。
- [repo][meta] 同步 dogfood、锁文件中的产品版本、说明和兼容性回归；不改变用户级 Hook 信任及项目业务实现。

## [1.12.0] - 2026-09-03

### Added

- [product][repo][meta] 支持项目自有 Rust Hook 的事务安装

## [1.11.0] - 2026-09-03

### Added

- [product] 项目自有 Rust Hook 使用显式注册，在受管锁定 workspace 的隔离快照中编译；二进制、注册、收据纳入升级事务，校验输入漂移及 Windows 无控制台入口，不回退 Python。
- [repo][meta] 同步工厂镜像并增加构建、注册、输入流和所有权回归；Assist 业务移植仅在隔离测试 worktree，正式下游迁移与发布仍须完成后续确认。

## [1.10.0] - 2026-09-03

### Added

- [product][repo][meta] 新增项目知识库层并保障升级资产保留

## [1.9.0] - 2026-09-03

### Added

- [product][repo] 新增固定第六层 `doc/5_project_knowledgebase/`：初始化与升级提供空目录，知识正文和话题索引保持项目所有权；允许 legacy Rule / Memory 经逐源确认迁入，仍校验索引、路径、hash 和事务回滚。
- [product][meta] 明确知识资料不套用交付验收或自动归档规则；公共 AGENTS、索引与迁移指引同步。兼容性基线保持 1.8.6。
