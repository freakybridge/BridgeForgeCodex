...
| IA-14 | P2 | 缺少真实用户理解度与运行时行为验收 | R2 / R6 源码修复与 source-path 复测通过；待发布后安装 runtime / 用户试用 / 下游 |
...
当前验收口径、测试样本和逐项收据见 [`IA-14-user-runtime-acceptance.md`](IA-14-user-runtime-acceptance.md)。P0 首次运行发现并修正测量器把 SessionStart Show State 混算为每轮 prompt 上下文的问题；当前确定性基线为每轮 0 字符、SessionStart 固定夹具 80 字符。盲读代理与用户结果可行动性均为 5/5；首轮 12 组 runtime 严格统计为 7 组通过、2 组安全替代通过、3 组失败。R2 / R6 已在工厂源 1.5.18 修复，source-path forward test 4/4；受管分发器拒绝未提交工作树且零写入，故发布后安装 runtime、用户最终试用和真实下游仍未执行。
