...
| IA-14 | P2 | 缺少真实用户理解度与运行时行为验收 | R6 已安装 runtime 2/2；R2 在 1.5.23 安装版 0/2、1.5.24 工作树修复后 2/2；待发布安装复测 / 用户试用 / 下游 |
...
当前验收口径、测试样本和逐项收据见 [`IA-14-user-runtime-acceptance.md`](IA-14-user-runtime-acceptance.md)。P0 首次运行发现并修正测量器把 SessionStart Show State 混算为每轮 prompt 上下文的问题；当前确定性基线为每轮 0 字符、SessionStart 固定夹具 80 字符。盲读代理与用户结果可行动性均为 5/5；首轮 12 组 runtime 严格统计为 7 组通过、2 组安全替代通过、3 组失败。1.5.23 已通过公开安装器传播到用户级：R6 安装后 2/2，R2 因自然语言入口不经过 `$plan` 仍为 0/2。1.5.24 已将范围硬闸上移到公共 AGENTS，工作树 R2 2/2；发布安装复测、用户最终试用和真实下游仍未执行。
