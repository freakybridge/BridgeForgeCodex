...
> 本区由项目完全所有，bridgeforge-codex 更新时必须逐字保留。

### 项目架构红线

<!-- [必填] 写明本项目稳定的模块职责、依赖方向、数据流、外部副作用和风险/合规边界；填好后删除本注释。 -->

### 项目业务与安全红线

<!-- [按需] 写本项目特有的业务、数据、安全、风控、合规和外部系统边界；填好后删除本注释。 -->

### 项目目录地图

<!-- [必填] 写清核心源码、程序入口、配置入口、测试入口和文档入口；填好后删除本注释和所有 `<...>` 占位。 -->
...
| 程序入口 | `<path>` | `<职责>` |
| 配置入口 | `<path>` | `<职责>` |
| 测试入口 | `scripts/tests/` | `<职责>` |
| 项目文档 | `doc/README.md` | 唯一索引 |

### 项目快速命令

<!-- [必填] 填写项目真实可运行的初始化、检查、测试、构建和运行命令；填好后删除本注释和所有 `<command>` 占位。 -->
...
<!-- [按需] 列出项目自有的嵌套 AGENTS.md 及其适用目录；该索引不替代 Codex 原生层级加载。 -->
